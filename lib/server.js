...
const startServer = options => {
	...
	const conf = $lodash.merge({
		...
		template: '',
		// Ignore file path, relative to mock
		ignore: '',
	...
	middleware.forEach(item => {
		...
		if ($type(item) !== 'object') {
			console.log($chalk.red('[spore-mock] middleware must be an object'));
			return;
		}
		...
		if ($type(item.handle) === 'function') {
			if (typeof item.route === 'string') {
				...
				console.log($chalk.yellow('[spore-mock] middleware.route is not a string'));
				app.use(item.handle);
			}
		} else {
			console.log($chalk.red('[spore-mock] middleware.handle must be an object'));
			return;
		}
	});
	...
	proxy.forEach(item => {
		...
		if ($type(item) !== 'object') {
			console.log($chalk.red('[spore-mock] proxy must be an object'));
			return;
		}

		if ($type(item.proxy) === 'object') {
			if (typeof item.route === 'string') {
				app.use($proxy(item.route, item.proxy));
			} else {
				console.log($chalk.yellow('[spore-mock] proxy.route is not a string'));
				app.use($proxy(item.proxy));
			}
		} else {
			console.log($chalk.red('[spore-mock] proxy.proxy must be an object'));
			return;
		}
	});
	...
	// Handle home page
	app.get(conf.routeAdmin, $mockIndex({
		debug: conf.debug,
		ip: conf.ip,
		port: conf.port,
		links: conf.links,
	...
	app.listen(conf.port, () => {
		console.log(
			$chalk.cyan(`[MOCK] Mock server started at http://${conf.ip}:${conf.port}`)
		);
		console.log(
			$chalk.cyan(`[MOCK] Check api list at http://${conf.ip}:${conf.port}${conf.routeAdmin}`)
		);
	});
