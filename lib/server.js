...
var $type = require('spore-kit-obj/src/type');
...
function startServer(options) {
	...
	middleware.forEach(function(item) {
		...
		if ($type(item) !== 'object') {
			console.error('[spore-mock] middleware must be an object');
			return;
		}
		...
		if ($type(item.handle) === 'function') {
			if (typeof item.route === 'string') {
				...
				console.warn('[spore-mock] middleware.route is not a string');
				app.use(item.handle);
			}
		} else {
			console.error('[spore-mock] middleware.handle must be an object');
			return;
		}
	});
	...
	proxy.forEach(function(item) {
		...
		if ($type(item) !== 'object') {
			console.error('[spore-mock] proxy must be an object');
			return;
		}
		...
		if ($type(item.proxy) === 'object') {
			if (typeof item.route === 'string') {
				...
				console.warn('[spore-mock] proxy.route is not a string');
				app.use($proxy(item.proxy));
			}
		} else {
			console.error('[spore-mock] proxy.proxy must be an object');
			return;
		}
	});
